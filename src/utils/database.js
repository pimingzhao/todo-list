...
const isHasStore = (name, storeName) => database[name].db.objecStoreNames?.contains(storeName)
...
export const getData = (storeName, name) => {
  isNull(name) && (name = baseDb.name)
  isNull(storeName) && (storeName = name)
  if (isHasStore(name, storeName)) {
    return new Promise((resolve, reject) => {
      const transaction = database[name].db.transaction(storeName, 'read')
      const store = transaction.objectStore(storeName)
